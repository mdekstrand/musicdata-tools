...
def process():
    ...
    brainz_size = conn.execute("select count(*) from musicbrainz.mb_artist"
                                ).fetchone()[0]
    ##
    ...
    for mlhd_path in tqdm(segments, desc="Processing Directories", unit="dir"):
        ...
        data_intial_size += conn.execute(f"""select count(*) from 
                                        read_parquet('{mlhd_path}/*.parquet')""").fetchone()[0]
    ...
    with open(log_path, 'a') as logfile:
            ...
            logfile.write(
                f"# of rows in initial data: {data_intial_size:,}\n") 
            logfile.write(
                f"# of duplicate rows removed: {duplicate_count:,}\n")  
            logfile.write(
