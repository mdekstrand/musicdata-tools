...
COUNT_BAR_FORMAT = u'{desc}{desc_pad}{percentage:3.0f}%|{bar}| ' \
                 u'{count:{len_total}d}/{total:d} ' \
                 u'[{elapsed}<{eta}, {rate:.2f}{unit_pad}{unit}/s]'

FILE_BAR_FORMAT = '{desc}{desc_pad}{percentage:3.0f}%|{bar}| {count:!.2j}{unit} / {total:!.2j}{unit} ' \
...
def main(args: argparse.Namespace):
    ...
    fpb = progress.counter(total=len(files), desc='Input files', unit='files', bar_format=COUNT_BAR_FORMAT)
    ...
    if jobs == 1:
        ...
        with ProcessPoolExecutor(jobs, ctx, initializer=_init_logging, initargs=(pg_queue,level)) as pool:
    ...
    with open(file, 'rb') as srcf, tarfile.TarFile(fileobj=srcf) as tf:
        for entry in tf:
            ...
            with tf.extractfile(entry) as cstr, decomp.stream_reader(cstr) as data:
                tbl = csv.read_csv(
                    data,
                    csv.ReadOptions(column_names=['timestamp', 'artist_ids', 'release_id', 'rec_id']),
                    csv.ParseOptions(delimiter='\t')
                )

            tbl = rec.db.from_arrow(tbl)
            proj = rec.db.sql(f"select '{uid}', timestamp, string_split(artist_ids, ','), release_id, rec_id from tbl")
            ...
            match action:
                case 'start':
                    ...
                    self.counters[name] = progress.counter(
                        total=float(size), desc=name,
                        unit='B', bar_format=FILE_BAR_FORMAT
                    )
                case 'update':
                    name, incr = payload
                    try:
